...
## 0. 들어가기 전에 — 핵심 용어
- **중첩 클래스(nested class)**: 클래스 안에 정의한 클래스. 크게 static 중첩 / inner / 지역 / 익명으로 나뉜다.
- **static nested class**: 바깥 인스턴스와 무관(이름공간만 안쪽). `new Outer.Nested()`로 바로 생성.
- **inner class(내부 클래스, non-static)**: 항상 특정 바깥 인스턴스에 소속. 바깥 인스턴스 필드에 직접 접근.
- **익명 클래스(anonymous class)**: 이름 없이 그 자리에서 인터페이스/클래스를 즉석 구현해 객체 하나를 만드는 것.
- **람다(lambda)**: 익명 클래스를 더 짧게 쓴 형태(함수형 인터페이스 대상). 익명 클래스의 '후신' — PART 5와 연결.

한 줄 그림: **클래스 안에 클래스를 둘 수 있고, "바깥 인스턴스에 묶이느냐"로 성격이 갈린다. 익명 클래스는 람다의 전신이다.**
