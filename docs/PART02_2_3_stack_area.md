...
## 0. 들어가기 전에 — 핵심 용어
- **Stack(스택)**: 메서드 호출 정보를 쌓아 올리는 메모리 영역. 스레드마다 하나씩 따로 있다.
- **스택 프레임(stack frame)**: 메서드 한 번 호출마다 생기는 칸. 그 메서드의 지역 변수·매개변수·중간 계산을 담는다.
- **LIFO(Last In First Out)**: 마지막에 쌓인 게 먼저 빠지는 구조. 메서드 호출은 쌓이고(push), 끝나면 위에서부터 제거(pop)된다.
- **StackOverflowError**: 스택 프레임이 너무 많이 쌓여(예: 끝없는 재귀) Stack 공간을 넘으면 나는 에러.
- **스레드별(thread-local)**: Stack은 스레드마다 독립적이라, 지역 변수는 다른 스레드와 공유되지 않는다(그래서 스레드 안전 — PART 7).

한 줄 그림: **메서드를 부르면 스택 프레임이 쌓이고(push) 끝나면 빠진다(pop). Stack은 스레드마다 따로라 지역변수는 안전하다.**
