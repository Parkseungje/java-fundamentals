...
## 0. 들어가기 전에 — 핵심 용어
- **메서드 실행 메커니즘**: "메서드 호출 한 줄"이 JVM 내부에서 어떻게 처리되는지(어떤 코드를 찾아 어디서 실행하는지).
- **정적 바인딩(static binding)**: 어떤 메서드를 부를지 **컴파일 시점**에 결정(예: static·private·final 메서드, 오버로딩).
- **동적 바인딩(dynamic binding)**: **런타임**에 실제 객체 기준으로 결정(오버라이딩된 인스턴스 메서드 — 1.5 다형성).
- **오버로딩(overloading)**: 같은 이름, 다른 매개변수의 메서드를 여러 개 두는 것(컴파일러가 인자로 구분).
- **오버라이딩(overriding)**: 자식이 부모 메서드를 재정의(런타임에 실제 객체 것이 실행).
- **Method Area / 스택 프레임**: 메서드 '코드(틀)'는 Method Area에 한 벌, 호출할 때마다 실행용 '프레임'은 Stack에 생긴다(2.2/2.3).

한 줄 그림: **메서드 코드는 Method Area에 한 벌 있고, 호출하면 Stack에 프레임을 만들어 실행한다. '어느 메서드냐'는 정적/동적 바인딩으로 결정된다.**
