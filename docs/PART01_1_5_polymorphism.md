...
## 0. 들어가기 전에 — 핵심 용어
- **다형성(polymorphism)**: 한 부모 타입 변수가 여러 자식 객체를 가리키고, 같은 메서드 호출이 실제 객체에 따라 다르게 동작하는 성질.
- **오버라이딩(overriding)**: 자식이 부모 메서드를 자기 방식으로 재정의하는 것.
- **컴파일 타임 타입(변수 타입)**: 코드에 적힌 변수의 선언 타입(예: `Animal a`의 `Animal`).
- **런타임 타입(실제 객체 타입)**: a가 실제로 가리키는 객체의 타입(예: `new Dog()`의 `Dog`).
- **동적 바인딩(dynamic binding)**: 어떤 메서드가 실행될지를 런타임의 '실제 객체' 기준으로 정하는 것.
- **업캐스팅/다운캐스팅**: 자식→부모 타입으로 보기(안전) / 부모→자식으로 되돌리기(주의 필요).
- **`instanceof`**: 객체가 특정 타입인지 검사하는 연산자(다운캐스팅 전 안전 확인용).

한 줄 그림: **`Animal a = new Dog()`에서 변수 타입은 Animal이지만, `a.makeSound()`는 실제 객체 Dog의 것이 실행된다(동적 바인딩).**
