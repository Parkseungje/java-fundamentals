...
## 0. 들어가기 전에 — 핵심 용어
- **List**: 순서가 있고 중복을 허용하는 컬렉션. 인덱스(0,1,2...)로 접근.
- **ArrayList**: 내부가 **배열**. 인덱스 접근이 빠르지만(O(1)), 중간 삽입/삭제는 뒤 원소를 밀어야 해 느림.
- **LinkedList**: 내부가 **연결 리스트**(노드들이 앞뒤로 연결). 중간 삽입/삭제는 빠르지만, 인덱스 접근은 처음부터 따라가야 해 느림.
- **Vector**: ArrayList의 옛 버전. 메서드가 `synchronized`라 스레드 안전하지만 느려서 잘 안 씀.
- **시간 복잡도(Big-O)**: 입력 크기에 따른 연산 비용 표기. O(1)=일정, O(n)=원소 수에 비례.
- **동적 배열(dynamic array)**: 꽉 차면 더 큰 배열로 자동 확장(resize)하는 배열. ArrayList의 내부 동작.

한 줄 그림: **ArrayList(배열) = 인덱스 접근 빠름·중간 삽입 느림 / LinkedList(연결) = 중간 삽입 빠름·인덱스 접근 느림. 대부분 ArrayList를 기본으로 쓴다.**
