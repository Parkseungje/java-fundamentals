...
## 0. 들어가기 전에 — 핵심 용어
- **바이트코드(bytecode)**: 자바 소스(`.java`)를 컴파일하면 나오는 `.class` 안의 중간 언어. JVM이 읽어 실행한다(특정 CPU에 종속 안 됨).
- **JVM 명령어(opcode)**: 바이트코드를 이루는 한 줄 명령(예: `iload`, `invokevirtual`, `new`). 스택 기반으로 동작.
- **오퍼랜드 스택(operand stack)**: 바이트코드가 계산할 때 값을 잠깐 올려두는 작업대(스택 프레임 안에 있음).
- **상수 풀(Constant Pool)**: 클래스가 쓰는 상수·이름(문자열, 클래스명, 메서드명, 필드명 등)을 모아둔 표.
- **심볼 참조(symbolic reference)**: 바이트코드가 다른 클래스/메서드를 '실제 주소'가 아니라 '이름'으로 가리키는 것. 실행 시점에 실제 대상으로 연결(resolution)된다.
- **javap**: `.class`의 바이트코드·상수 풀을 사람이 읽게 풀어 보여주는 도구(`-c` 바이트코드, `-v` 상수 풀까지).

한 줄 그림: **소스 → 바이트코드(.class)로 컴파일되고, JVM이 명령어를 오퍼랜드 스택 위에서 실행한다. 이름들은 상수 풀에 모여 심볼 참조로 연결된다 — javap로 직접 볼 수 있다.**
