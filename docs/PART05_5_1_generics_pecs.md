...
## 0. 들어가기 전에 — 핵심 용어
- **제네릭(generics)**: 담을/다룰 타입을 `<T>`처럼 파라미터로 받는 문법. 타입 안전(컴파일 때 타입 체크)과 재사용을 동시에.
- **타입 파라미터**: `List<T>`의 `T` 같은 자리표. 관례적으로 T(Type)·E(Element)·K(Key)·V(Value) 등을 쓴다.
- **불공변(invariant)**: `List<Integer>`는 `List<Number>`의 하위 타입이 **아니다**. 제네릭은 기본적으로 이렇게 엄격하다.
- **와일드카드(`?`)**: "어떤 타입인지 모르지만 무엇이든"을 뜻하는 자리. 불공변의 제약을 유연하게 푼다.
- **`? extends T`(상한)**: "T이거나 T의 자식". 데이터를 **꺼내(읽기)** 쓸 때 — 생산자(Producer).
- **`? super T`(하한)**: "T이거나 T의 부모". 데이터를 **넣을(쓰기)** 때 — 소비자(Consumer).
- **PECS**: "Producer-Extends, Consumer-Super". 읽으면 extends, 쓰면 super를 쓰라는 기억법.

한 줄 그림: **제네릭은 타입을 파라미터로 받아 타입 안전을 준다. 단 불공변이라, 유연성이 필요하면 와일드카드를 쓰고 그 방향은 PECS(읽기=extends, 쓰기=super)로 정한다.**
