...
## 0. 들어가기 전에 — 핵심 용어
- **런타임 데이터 영역(Runtime Data Area)**: JVM이 프로그램 실행 중 데이터를 담아두는 메모리 공간 전체. 아래로 나뉜다.
- **Method Area(메서드 영역)**: 클래스 정보(필드·메서드 구조), static 변수, 상수 풀이 올라가는 곳. 모든 스레드가 공유.
- **Heap(힙)**: `new`로 만든 **객체**가 사는 곳. 모든 스레드가 공유. GC(가비지 컬렉션)의 대상.
- **Stack(스택)**: 메서드 호출마다 쌓이는 '스택 프레임'(지역 변수·중간 계산)이 있는 곳. **스레드마다 따로**.
- **PC Register / Native Method Stack**: 각 스레드의 '지금 실행 중인 명령 위치' / 네이티브(C 등) 호출용 스택. (스레드별)
- **공유 vs 스레드별**: Method Area·Heap = 공유(그래서 동시성 문제 발생 — PART 7), Stack·PC = 스레드별(그래서 안전).

한 줄 그림: **JVM 메모리는 공유 영역(Method Area·Heap)과 스레드별 영역(Stack·PC)으로 나뉜다. 객체는 Heap, 지역변수는 Stack.**
