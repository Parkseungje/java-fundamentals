...
## 0. 들어가기 전에 — 핵심 용어
- **GC 종류(collector)**: 3.3의 알고리즘들을 실제로 조합해 만든 'GC 구현체'. JVM 옵션(`-XX:+UseXxxGC`)으로 고른다.
- **처리량(throughput)**: 전체 시간 중 '애플리케이션이 실제 일한' 비율(GC에 안 뺏긴 시간). 높을수록 좋음.
- **지연/멈춤 시간(latency, pause time)**: GC 때문에 앱이 멈추는(STW) 시간. 짧을수록 좋음(응답성).
- **Stop-The-World(STW)**: GC가 도는 동안 앱 스레드를 멈추는 것. 이 시간을 줄이는 게 최신 GC의 목표.
- **동시(concurrent) GC**: 앱을 멈추지 않고 GC 작업의 상당 부분을 '같이' 돌리는 방식(STW 최소화).
- 대표 GC: **Serial**(단일 스레드, 작은 앱) → **Parallel**(여러 스레드, 처리량 중시) → **CMS**(저지연, 폐기됨) → **G1**(Java 9+ 기본, 균형) → **ZGC/Shenandoah**(초저지연, 수 ms STW).

한 줄 그림: **GC는 '처리량 ↔ 멈춤 시간'의 트레이드오프를 두고 발전해 왔다. Serial→Parallel→G1→ZGC로 갈수록 STW를 줄이는 방향이다.**
