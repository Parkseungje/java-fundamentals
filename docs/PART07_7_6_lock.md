...
### 1-1. 락 메커니즘 개념 정리 — 뮤텍스 · 세마포어 · 모니터 · 스핀락
구체 도구(ReentrantLock 등)에 들어가기 전에, 면접·문서에서 자주 나오는 네 가지 동기화 메커니즘 용어를
정리하자. 넷 다 "어떻게 상호 배제(또는 대기)를 구현하느냐"의 종류다.

**뮤텍스(Mutex = MUTual EXclusion)** — "한 번에 딱 한 스레드만" 들어가게 하는 자물쇠(열쇠 1개).
- 자바의 `synchronized`·`ReentrantLock`이 사실상 뮤텍스다.
- 비유: 1인용 화장실 — 열쇠 하나, 한 명만.

**세마포어(Semaphore)** — "한 번에 N개까지 허용"하는 카운팅 락(허가증 permit이 N개).
- N=1이면 뮤텍스와 같고(이진 세마포어), N≥2면 **동시 접근 수 제한**에 쓴다(예: DB 커넥션 5개만, 동시 다운로드 3개만).
- 자바: `java.util.concurrent.Semaphore` — `acquire()`(허가 얻기, 없으면 대기) / `release()`(반납).
```java
Semaphore sem = new Semaphore(3);     // 동시 3개까지 허용
sem.acquire();                         // 허가 1개 얻기(다 찼으면 대기)
try {
    // 보호 자원 사용 (동시에 최대 3스레드)
} finally {
    sem.release();                     // 허가 반납 (반드시 finally)
}
```
- 비유: 주차장 5칸 — 5대까지 입차, 6번째는 빈자리 날 때까지 대기.

**모니터(Monitor)** — **락(상호 배제) + 조건 변수(wait/notify)를 하나로 묶은** 고수준 메커니즘.
- 자바에선 **모든 객체가 모니터**를 갖는다. `synchronized`(상호 배제) + `wait/notify`(조건 대기, 7.7)가
  바로 모니터의 두 기능이다. 그래서 7.5의 '모니터 락'이 곧 이 모니터를 가리킨다.
- 즉 뮤텍스가 "한 명만"만 한다면, 모니터는 거기에 "조건이 될 때까지 기다렸다 깨우기"까지 합친 것.
- 비유: 회의실(락) + 그 앞 대기실(조건 변수)을 한 묶음으로 제공.

**스핀락(Spinlock)** — 락을 못 얻으면 **잠들지 않고** "얻었나? 얻었나?" 하며 **CPU를 돌리며(busy-wait/스핀)
기다리는** 락.
- 장점: 락이 **아주 짧게** 잡힐 상황이면, 잠들고 깨우는 비용(컨텍스트 스위칭)보다 잠깐 도는 게 더 싸다.
- 단점: 오래 기다리면 CPU를 헛되이 태운다. 보통 CAS(7.5)로 구현하며, Atomic의 'CAS 실패 시 재시도 루프'가 스핀의 일종이다.
- 비유: 화장실 앞에서 앉아 기다리지(잠듦) 않고, 문을 계속 두드리며 서 있는 것.

| 개념 | 한 줄 정의 | 자바에서 |
|---|---|---|
| 뮤텍스 | 한 번에 1개만(상호 배제) | `synchronized`, `ReentrantLock` |
| 세마포어 | 한 번에 N개까지(카운팅) | `Semaphore` |
| 모니터 | 뮤텍스 + 조건 대기(wait/notify) | 모든 객체의 모니터(synchronized + wait/notify) |
| 스핀락 | 잠들지 않고 돌며 대기 | CAS 기반(Atomic 재시도 루프 등) |

> 정리: 대기 '인원수'로 보면 뮤텍스(1) ⊂ 세마포어(N), 대기 '방식'으로 보면 잠들기(블로킹 락) ↔ 돌기(스핀락),
> 그리고 모니터는 "락 + 조건 변수"를 합친 상위 개념이다. 아래 ReentrantLock은 '뮤텍스 + 조건(Condition)'을
> 명시적으로 제공하는 구현이라 볼 수 있다.

### 1-2. LockSupport — 락의 바닥에 있는 저수준 부품
...
### 1-3. ReentrantLock — 직접 잠그고 푸는 '명시적 락' (실무 표준)
...
- **Q. unlock이 try '안'에 있으면 왜 실행이 안 되나?**
  - 내 답: 예외가 나면 자바는 try 블록의 '그 줄 이후를 전부 건너뛰고' 예외를 위로 던진다. 그래서 그
    아래의 unlock은 도달조차 못 한다. finally는 무조건 실행되므로 거기 둬야 한다. (1-3)
...
- **Q. "Reentrant(재진입)"가 무슨 뜻인가?**
  - 내 답: 같은 스레드가 이미 쥔 락을 다시 lock()해도 막히지 않는 것(획득 횟수를 세어 그만큼 unlock해야
    완전히 풀림). 락을 쥔 메서드가 같은 락을 쓰는 다른 메서드를 호출해도 자기 자신에게 안 막힌다. (1-3)

- **Q. 데드락의 근본 해법과 임시 회피책의 차이는?**
  - 내 답: 근본 해법 = 모든 스레드가 락을 같은 순서로 잡기(순환 대기 제거). 회피책 = tryLock(시간)으로
    못 얻으면 쥔 락 풀고 재시도(점유하며 대기 제거). 가능하면 순서 통일이 우선. (1-4)

- **Q. synchronized도 충분한데 ReentrantLock을 쓰는 경우는?**
  - 내 답: "못 얻으면 포기/타임아웃"이 필요하거나(tryLock), 대기 중 취소가 필요하거나(lockInterruptibly),
    공정성이 필요할 때. 단순한 보호면 synchronized가 더 간결하고 안전(자동 해제)하다. (1-0, 1-3)

- **Q. 뮤텍스 / 세마포어 / 모니터 / 스핀락의 차이는?**
  - 내 답: 뮤텍스=한 번에 1개만(상호 배제), 세마포어=N개까지(카운팅, 동시 접근 수 제한), 모니터=뮤텍스+
    조건 대기(wait/notify)를 합친 것(자바 synchronized의 바탕), 스핀락=잠들지 않고 돌며 기다리는 락(짧은
    대기에 유리, CAS 기반). (1-1)

- **Q. 세마포어는 자바에서 어떻게 쓰나?**
  - 내 답: `new Semaphore(N)`으로 허가 N개를 만들고, `acquire()`로 얻고(없으면 대기) `release()`로 반납한다
    (반드시 finally). N=1이면 뮤텍스, N≥2면 동시 접근 수 제한(커넥션 풀 등). (1-1)
