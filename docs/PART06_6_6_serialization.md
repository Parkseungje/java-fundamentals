...
### ★ 중요 — 자바 고유 직렬화는 웹 개발에서 거의 안 쓴다 (JSON을 쓴다)
이 단원에서 배우는 `Serializable`/`ObjectOutputStream`은 **자바 '고유(native)' 직렬화**다. 핵심을
분명히 하자: **"직렬화"라는 개념은 웹에서 매일 쓰지만, 자바 고유 직렬화는 현대 웹 개발에서 거의 안
쓰이고 오히려 지양된다.** 둘을 구분해야 한다.

- **"직렬화"라는 개념 = 객체 ↔ 전송/저장 가능한 형태로 변환.** 이건 어디에나 있다. 웹 API에서 객체를
  주고받는 것도 직렬화/역직렬화다. 다만 그 '형태'가 자바 바이트가 아니라 보통 **JSON**이다.
- **자바 고유 직렬화(`Serializable`) = 자바 바이트 형태로 변환.** 이건 **양쪽이 모두 자바(JVM)** 여야만
  읽을 수 있다. 그래서 "JVM ↔ JVM 통신/저장" 전용이다.

**웹 개발자가 실제로 쓰는 건 JSON 직렬화다.** 예: 클라이언트(브라우저/모바일/다른 언어 서버)와
REST API로 통신할 때, 스프링은 객체를 **JSON 문자열**로 바꿔 보낸다(Jackson 라이브러리가 함).

```java
// 웹에서 실제로 쓰는 직렬화 = JSON (Jackson). Serializable과 무관.
@GetMapping("/users/1")
public User getUser() { return new User("홍길동", 30); }
// -> 스프링이 자동으로 {"name":"홍길동","age":30} (JSON 문자열)로 직렬화해 응답
```

**왜 자바 고유 직렬화를 웹에서 피하나:**
- **상대가 자바가 아닐 수 있다**: 브라우저(JS)·모바일·파이썬 서버는 자바 바이트를 못 읽는다. JSON은
  언어 중립이라 누구나 읽는다.
- **보안 위험**: 신뢰할 수 없는 데이터를 `readObject`로 역직렬화하면 악성 객체가 만들어져 코드 실행
  공격(역직렬화 취약점)으로 이어질 수 있다 — 유명한 보안 사고가 많다.
- **버전 취약성**: serialVersionUID 관리(아래)가 까다로워 클래스 변경에 약하다.
- **사람이 못 읽음**: 바이너리라 디버깅·로깅이 불편하다(JSON은 텍스트라 눈으로 읽힘).

**그럼 자바 고유 직렬화는 어디서 쓰나(소수):** 양쪽이 다 자바인 내부 환경 — RMI, 일부 분산 캐시/세션
복제, 옛 프레임워크 등. 신규 개발에선 대부분 JSON(외부 통신)·또는 Protobuf/Avro(고성능 내부 통신)를 쓴다.

정리: **이 단원은 "직렬화의 원리"를 자바 고유 방식으로 배우는 것**이고(역직렬화가 생성자를 안 거친다,
transient, 버전 관리 같은 개념은 JSON에도 통하는 통찰이다), **실무 웹에서 객체를 주고받을 땐 JSON을
쓴다**고 기억하면 된다. 자바 고유 직렬화 자체는 "JVM끼리"라는 좁은 경우에만 등장한다.
