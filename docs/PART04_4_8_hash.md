...
## 0. 들어가기 전에 — 핵심 용어
- **해시(hash)**: 임의의 데이터를 고정 크기의 숫자로 바꾸는 것. 그 숫자로 '저장 위치'를 빠르게 정한다.
- **hashCode()**: 객체를 정수 해시값으로 바꾸는 메서드. 어느 버킷에 넣을지 정하는 데 쓴다.
- **버킷(bucket)**: 해시값으로 정해지는 저장 칸(내부 배열의 한 칸). HashMap/HashSet의 실제 저장 단위.
- **해시 충돌(collision)**: 서로 다른 키가 같은 버킷에 배정되는 것. 피할 수 없어 처리 방법이 필요.
- **체이닝(chaining)**: 한 버킷에 여러 원소를 연결 리스트(또는 트리)로 매다는 충돌 처리 방식.
- **LoadFactor(부하율)**: (원소 수 / 버킷 수). 이 값이 임계치(기본 0.75)를 넘으면 버킷을 늘려(resize) 충돌을 줄인다.
- **hashCode-equals 계약**: "equals가 true면 hashCode도 같아야" 한다. 안 지키면 HashMap/HashSet이 오작동(같은 객체를 못 찾음).

한 줄 그림: **해시는 hashCode로 버킷 위치를 정해 O(1) 검색을 한다. 충돌은 체이닝으로 풀고, 너무 차면 resize한다. 그래서 키 객체는 hashCode/equals를 올바르게 같이 재정의해야 한다.**
