...
## 0. 들어가기 전에 — 핵심 용어
- **Comparable**: 클래스가 '자기 자신의 기본 정렬 기준(자연 순서)'을 정하는 인터페이스. `compareTo(o)` 하나를 구현.
- **Comparator**: 클래스 '바깥에서' 별도의 비교 기준을 정의하는 인터페이스. `compare(o1, o2)` 구현. 여러 기준을 만들 수 있다.
- **자연 순서(natural ordering)**: 그 클래스가 스스로 정한 단 하나의 기본 정렬 순서(Comparable로 정함).
- **반환값 계약**: 비교 메서드는 'this가 작으면 음수, 같으면 0, 크면 양수'를 반환해야 한다(자바 명세가 글로 정한 약속).
- **`Collections.sort` / TreeSet / TreeMap**: 이 비교 기준을 사용해 정렬·정렬 저장을 한다.
- **콤비네이터**: `comparing`, `comparingInt`, `reversed`, `thenComparing` 등 Comparator를 만들고 조합하는 메서드들.

한 줄 그림: **정렬하려면 '비교 기준'이 있어야 한다 — 클래스 안에 하나면 Comparable(자연 순서), 밖에서 여러 개면 Comparator. 둘 다 '음수/0/양수' 계약을 따른다.**
