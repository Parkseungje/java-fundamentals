...
## 0. 들어가기 전에 — 핵심 용어
- **SOLID**: 변경에 강한 객체지향 설계 5원칙의 머리글자 모음(SRP·OCP·LSP·ISP·DIP).
- **SRP(단일 책임)**: 클래스가 바뀌어야 할 이유는 하나여야 한다.
- **OCP(개방-폐쇄)**: 확장엔 열리고 수정엔 닫혀야 한다(새 기능을 기존 코드 수정 없이 추가).
- **LSP(리스코프 치환)**: 자식은 부모 자리에 넣어도 문제없이 동작해야 한다.
- **ISP(인터페이스 분리)**: 안 쓰는 메서드까지 강요하는 큰 인터페이스를 작게 쪼개라.
- **DIP(의존 역전)**: 구체 클래스가 아니라 추상(인터페이스)에 의존하라.
- 관련 용어 — **결합도(coupling)**: 모듈 간 의존 정도(낮을수록 좋음). **추상화에 의존**: 인터페이스를 보고 코딩.

한 줄 그림: **SOLID는 "변경이 닥쳐도 안 무너지는 설계"의 5계명이다.** 이 원칙들이 PART 8(DI)·12(AOP)의 토대가 된다.
