 ...
 *
 * 이 예시가 답하려는 질문: 예시 2에서는 그래도 "함수를 통해" 데이터를 바꿨다. 그런데 함수 자체를
 * 무시하고 필드에 직접 값을 꽂아 넣으면? 절차지향과 객체지향은 이 "우회 시도"를 각각 어떻게 다루는가?
 *
 * 왜 이 시나리오인가: 예시 1·2는 "정상 통로(함수)를 거친 변경"을 다뤘다. 이 예시는 한 단계 더
 * 나아가 통로를 무시한 직접 접근을 시도한다. 이것이 캡슐화의 진짜 시험대다. 규칙을 함수에 잘
 * 넣어두더라도, 누군가 함수를 건너뛰고 필드를 직접 바꿀 수 있다면 그 규칙은 무의미해지기 때문이다.
 *
 * 예상 결과:
 *   절차지향: car.speed = 9999가 런타임에 조용히 성공한다. CarData의 필드가 public이라
 *            컴파일러도, 데이터 자신도 이를 막을 수단이 없다. 말도 안 되는 값이 그대로 들어간다.
 *   객체지향: car.speed = 9999는 애초에 컴파일이 되지 않는다(private 필드). 그래서 아래 해당 줄은
 *            주석 처리해 두었다 — 주석을 풀면 IDE/컴파일러가 즉시 에러를 낸다. 이 "컴파일 자체가
 *            안 됨"이야말로 자바가 언어 차원에서 캡슐화를 강제한다는 증거다.
 * -> 예시 2와의 차이: 예시 2는 "함수를 거치되 규칙이 흩어진" 문제였고, 예시 3은 "함수를 아예 안
 * 거치는" 더 근본적인 위협이다. 객체지향은 후자를 컴파일 단계에서 원천 차단한다.
 */
