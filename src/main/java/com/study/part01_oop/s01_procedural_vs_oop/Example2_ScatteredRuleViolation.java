 ...
 *
 * 이 예시가 답하려는 질문: "속도는 0 미만이 될 수 없다"는 규칙(invariant)을 코드에서 어떻게
 * 강제하는가? 절차지향에서 이 규칙을 한 함수에는 넣고 다른 함수에는 깜빡하면 어떤 일이
 * 벌어지는가? 객체지향은 왜 그런 사고를 구조적으로 막는가?
 *
 * 왜 이 시나리오인가: 두 단계로 나눠 일부러 규칙을 시험한다.
 *   1단계 - 가속 50 -> 브레이크 80: 가속한 양보다 더 크게 감속해서 0 밑(음수) 경계를 넘겨본다.
 *           절차지향의 brake는 클램프가 있으므로 여기서는 둘 다 0으로 잘 막힌다.
 *   2단계 - applyPenalty 30: 같은 speed를 깎지만, 절차지향의 applyPenalty는 클램프를 "깜빡한"
 *           함수다. 같은 규칙이 함수마다 따로 관리되기 때문에 생기는 구멍을 노린다.
 *
 * 예상 결과:
 *   1단계: 절차지향·객체지향 모두 speed = 0 (절차지향도 brake에는 클램프가 있으므로).
 *   2단계: 절차지향은 speed = -30 (applyPenalty가 규칙을 안 지킴) /
 *          객체지향은 speed = 0 (모든 변경이 단일 통로 changeSpeedBy를 거쳐 규칙이 자동 적용).
