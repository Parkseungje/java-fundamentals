 ...
 *
 * <p><b>이 예시가 답하려는 질문</b>: 절차지향과 객체지향의 차이는 "평범하게 잘 쓸 때"부터
 * 드러나는가? 아니면 무언가 잘못될 때(규칙 위반·우회)에만 드러나는가?
 *
 * <p><b>왜 이 시나리오인가</b>: 일부러 규칙을 어기지 않는 안전한 입력만 사용한다
 * (가속 60 → 브레이크 20 → 최종 40, 한 번도 0 밑으로 가지 않음). 이렇게 하면 두 방식의
 * "결과"가 같은지 비교할 수 있다.
 *
 * <p><b>예상 결과</b>: 두 방식 모두 최종 speed = 40으로 <b>동일</b>하게 나올 것이다.
 * 즉 정상 흐름에서는 절차지향이든 객체지향이든 겉보기 결과에 차이가 없다.
 * → 이로써 "두 패러다임의 차이는 정상 동작이 아니라, 예시 2·3에서 다룰 <i>비정상 상황</i>에서
